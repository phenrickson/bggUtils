...
get_bgg_games <- function(game_ids,
  ...
  if (length(game_ids) < 500) {
    ...
    if (tidy == F) {
      out <- list(
        "game_ids" = bgg_games_xml$game_ids,
    ...
    resp_batches <-
      foreach(
        ...
        # get to list
        bgg_games_xml_obj <- list(
          "game_ids" = out[["game_ids"]],
    ...
    if (tidy == F) {
      ...
      out <- list(
        "game_ids" = map(resp_batches, ~ .x[["game_ids"]]) %>% unlist(),
        "problem_game_ids" = map(resp_batches, ~ .x[["problem_game_ids"]]) %>% unlist(),
        "bgg_games_data" = map(resp_batches, ~ .x[["bgg_games_data"]] %>%
          bind_rows() %>%
          toJSON())
      )
    }
...
extract_bgg_data <- function(input_parsed_bgg_api) {
  ...
  bgg_data[["description"]] <-
    unescape_html(simple_node_parse(
      input_parsed_bgg_api,
      "description"
    ))
...
get_bgg_games_xml <- function(game_ids) {
  ...
  if (resp_bgg_api$status_code == 200) {
